...
if (resourceTimer <= 0 && parentPlant.playerPlant) {
	...
	if (curXylemBlocks < xylemMax) {
		...
		if (resourceRoll > 0 && resourceRoll <= nutRatio) {
			//Generated Nutrients
			var newResource = instance_create_layer(self.x, self.y + SIZE/2, "FrontInstances", o_NutrientBlock); 
			...
			//Generated Water
			var newResource = instance_create_layer(self.x, self.y + SIZE/2, "FrontInstances", o_WaterBlock); 
			newResource.parentRoot = self; 
			newResource.startingParent = self; 
			if (parentPlant.currentTraits[TRAIT.XYLEM_1]) {
				newResource.transferTimerMax = PLANT_TIMER_XYLEMTRANSFER_MAX * 0.80	
			...
			//Generated Radiation	
			var newResource = instance_create_layer(self.x, self.y + SIZE/2, "FrontInstances", o_RadiationBlock); 
			newResource.parentRoot = self; 
			newResource.startingParent = self; 
			if (parentPlant.currentTraits[TRAIT.XYLEM_1]) {
				newResource.transferTimerMax = PLANT_TIMER_XYLEMTRANSFER_MAX * 0.80	
			...
			var newResource = instance_create_layer(self.x, self.y + SIZE/2, "FrontInstances", o_ContaminationBlock); 
			newResource.parentRoot = self; 
			newResource.startingParent = self;
			if (parentPlant.currentTraits[TRAIT.XYLEM_1]) {
				newResource.transferTimerMax = PLANT_TIMER_XYLEMTRANSFER_MAX * 0.80	
